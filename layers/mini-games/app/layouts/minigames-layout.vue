...
defineOptions({ name: 'MinigamesLayout' });
...
const title = computed(() => {
  ...
  return t('minigames.meta.defaultTitle');
});
...
type MinigameNavItem = {
  path: string;
  label: string;
  icon: string;
};

const minigameNavItems = computed<MinigameNavItem[]>(() => [
  {
    path: '/minigames',
    label: t('minigames.navigation.hub'),
    icon: 'lucide:layout-grid',
  },
  {
    path: '/minigames/dodge',
    label: t('minigames.navigation.dodge'),
    icon: 'lucide:rocket',
  },
]);
...
const isNavActive = (item: MinigameNavItem): boolean => {
  const target = item.path.replace(/\/$/, '') || '/';
  if (item.path === '/minigames') {
    return normalizedPath.value === target;
  }
  return normalizedPath.value.startsWith(target);
};
...
const toggleTheme = () => {
  ...
  if (typeof document !== 'undefined') {
    ...
    localStorage.setItem('neuraflow-minigames-classic-theme', isDark.value ? 'dark' : 'light');
  }
};
...
onMounted(() => {
  ...
  const saved = localStorage.getItem('neuraflow-minigames-classic-theme');
  if (saved === 'light') {
    isDark.value = false;
    ...
    >
      ...
      >
        ...
        <rect
          width="100%"
          height="100%"
          fill="url(#minigames-classic-grid)"
        />
      </svg>
    </div>
    <div
      ...
      >
        <div class="border-on-surface/[0.08] px-x-lg flex h-16 shrink-0 items-center justify-between border-b">
          ...
          <button
            ...
            :aria-label="t('minigames.navigation.closeMenu')"
            @click="isMobileSidebarOpen = false"
          >
            <Icon
        ...
        <div class="px-md py-x-lg flex flex-1 flex-col overflow-y-auto">
          <p class="text-body-x-sm mb-x-sm px-sm text-on-surface-dim font-semibold uppercase tracking-wider">
            {{ t('minigames.navigation.section') }}
          </p>
          <nav class="gap-xx-sm flex flex-col">
            <NuxtLink
              v-for="item in minigameNavItems"
              :key="item.path"
    ...
    <div class="relative z-10 flex h-full min-h-0 flex-col">
      ...
      >
        <div class="gap-md flex items-center">
          <button
            ...
            :aria-label="t('minigames.navigation.openMenu')"
        ...
        <div class="gap-sm flex items-center">
          <button
            type="button"
            class="p-xx-sm text-on-surface-dim duration-short hover:bg-on-surface/10 hover:text-on-surface rounded-lg transition-colors"
            :aria-label="isDark ? t('minigames.theme.switchToLight') : t('minigames.theme.switchToDark')"
            @click="toggleTheme"
          >
            <Icon
      ...
      <div class="flex min-h-0 flex-1 overflow-hidden">
        <aside
          ...
          role="navigation"
          :aria-label="t('minigames.navigation.section')"
        >
