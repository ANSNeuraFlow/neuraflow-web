...
defineOptions({ name: 'FocusExercisesLayout' });
...
const title = computed(() => {
  ...
  return t('focusExercises.meta.defaultTitle');
});
...
const navItems = computed<FocusNavItem[]>(() => [
  {
    path: '/focus-exercises',
    label: t('focusExercises.navigation.hub'),
...
const isNavActive = (item: FocusNavItem): boolean => {
...
const toggleTheme = () => {
  ...
  if (typeof document !== 'undefined') {
    ...
    localStorage.setItem('neuraflow-focus-exercises-theme', isDark.value ? 'dark' : 'light');
  }
};

onMounted(() => {
  document.body.classList.add('overflow-hidden');
  const saved = localStorage.getItem('neuraflow-focus-exercises-theme');
  if (saved === 'light') {
    isDark.value = false;
  ...
  <div class="bg-surface text-on-surface relative h-dvh max-h-dvh min-h-0 overflow-hidden">
    ...
    <Transition name="slide-sidebar">
      <aside
        ...
        role="navigation"
      >
        <div class="border-on-surface/[0.08] px-x-lg flex h-[6.8rem] shrink-0 items-center justify-between border-b">
          <NuxtLink
            :to="localePath('/')"
          ...
          <button
            ...
            :aria-label="t('focusExercises.navigation.closeMenu')"
            @click="isMobileSidebarOpen = false"
          >
            <Icon
        ...
        <div class="px-md py-x-lg flex flex-1 flex-col overflow-y-auto">
          <p class="text-body-x-sm mb-x-sm px-sm text-on-surface-dim font-semibold uppercase tracking-wider">
            {{ t('focusExercises.navigation.section') }}
          </p>
          <nav class="gap-xx-sm flex flex-col">
            <NuxtLink
              ...
              :class="isNavActive(item) ? 'bg-on-surface/[0.15] text-on-surface font-semibold' : ''"
              @click="isMobileSidebarOpen = false"
            >
              <Icon
                :name="item.icon"
    ...
    <div class="relative z-10 flex h-full min-h-0 flex-col">
      ...
      >
        <div class="gap-md flex items-center">
          <button
            ...
            :aria-label="t('focusExercises.navigation.openMenu')"
        ...
        <div class="gap-sm flex items-center">
          <button
            type="button"
            class="p-xx-sm text-on-surface-dim duration-short hover:bg-on-surface/10 hover:text-on-surface rounded-lg transition-colors"
            :aria-label="isDark ? t('focusExercises.theme.switchToLight') : t('focusExercises.theme.switchToDark')"
            @click="toggleTheme"
          >
            <Icon
      ...
      <div class="flex min-h-0 flex-1 overflow-hidden">
        <aside
          ...
          role="navigation"
          :aria-label="t('focusExercises.navigation.section')"
        >
